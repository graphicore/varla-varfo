...
import { getFromI18NConfigFull, localeToLabel, localeToValue, allLocalesFromTypeSpecI18N} from './typeSpec.mjs';
...
const LOCALE_SELECT_TEMPLATE = `
<div class="{klass}">
    <span class="like_a_label">{label}:</span><br />

    <select class="{klass}-select"/>
        <option value="DEFAULT">select by document</option>
    </select><br />

    <div class="{klass}-info">
        <span class="like_a_label {klass}-curent_content_language_code"
            >document language code:</span>&nbsp;<span class="{klass}-curent_content_language_code-value"
                >{curentContentLanguageCode}</span><br />

        <span class="like_a_label {klass}-config_locale"
            >configured locale:</span>&nbsp;<span class="{klass}-current_config_locale-value"
                >{currentConfigLocale}</span><br />

        <span class="like_a_label {klass}-applied_config_locale"
            >applied locale:</span>&nbsp;<span class="{klass}-applied_config_locale-value"
                >{appliedConfigLocale}</span><br />
    <div>
</div>
`;

class LocaleSelectWidget {
    /* localeSelectWidgetSettings = {
     *     typeSpecI18N,
     *     updateAfterChangedContent, // fn(langCode, locale)
     *     curentContentLanguageCode,
     *     currentConfigLocale,
     *     appliedConfigLocale
     * }
     */
    constructor(domTool, container, templateVars_, localeSelectWidgetSettings) {
        this._domTool = domTool;
        this.container = container;
        this._templateVars = templateVars_;
        this._settings = localeSelectWidgetSettings;

        var template = LOCALE_SELECT_TEMPLATE;

        const templateVars = Object.assign({}, templateVars_, {
            curentContentLanguageCode: this._settings['curentContentLanguageCode'],
            currentConfigLocale: localeToLabel(this._settings['currentConfigLocale']),
            appliedConfigLocale: localeToLabel(this._settings['appliedConfigLocale'])
        });
        ...
        this._valueToLocale = new Map();
        {
            const select = this.container.querySelector(`.${templateVars.klass}-select`)
              , allLocales = allLocalesFromTypeSpecI18N(this._settings.typeSpecI18N)
              , options = []
              ;

            for(const locale of allLocales) {
                const label = localeToLabel(locale)
                  , value = localeToValue(locale)
                  , option = this._domTool.createElement('option', {value}, label);
                  ;
                this._valueToLocale.set(value, locale);
                options.push(option);
            }
            select.append(...options);

            const configLocaleValue = localeToValue(this._settings.currentConfigLocale);

            select.value = this._valueToLocale.has(configLocaleValue)
                ? configLocaleValue
                : 'DEFAULT'
                ;
            select.addEventListener('change', (/*evt*/)=>{
                ...
                if(select.value === 'DEFAULT')
                    langCode = this._settings.curentContentLanguageCode;
                else
                    locale = this._valueToLocale.get(select.value);

                console.warn('setting langCode', langCode, 'locale', locale);
                this._settings.updateAfterChangedContent(langCode, locale);
            });
        }
    }

    reset() {
        this._settings.updateAfterChangedContent(this.settings.curentContentLanguageCode, null);
    }
}
...
class PortalAugmentationWidget extends _ContainerWidget {
    ...
    constructor(domTool, baseElement, id, targetElement, justificationController
        ...
        let widgetsConfig = [
            [
               LocaleSelectWidget, {
                      klass: `${klass}-type_spec-change`
                    , label: 'Change TypeSpec Script/Language'
                },
                localeSelectWidgetSettings
            ],
            '<hr />',
...
function selectTypeSpecByLocale(typeSpecI18N, locale) {
    // this is a stub
    ...
    const [configLocale, columnConfig] = getFromI18NConfigFull(typeSpecI18N.columnConfigI18N, locale)
    return [configLocale, {
        columnConfig
...
// scope of it. I.e. does it manage multiple justification controllers,
// e.g. on a demo/documentation page, we may want to have different
// configurations at the same time, similarly, on a complex (newspaper,
// magazine, etc.) website .
export function main(
    ...
    let justificationController = null
      ...
      , wdthJustificationSpecs = null
        // TODO: look into document on load; Default 'en'
      , curentContentLanguageCode = 'en'
      , currentConfigLocale = null
      ;

/*
contentLanguageCode => look into document on load OR default en
contentLanguageCode => set by wikipediaWidget => contentDocument lang code
        => that widget can replace the content document, so it can set that code
userConfigLocale => set by LocaleSelectWidget
appliedConfigLocale => userConfigLocale || contentLanguageCode



contentLanguageCode currentConfigLocale
=> the widget should not change the contentLanguageCode
=> just change the applied locale!
=> hence updateAfterChangedContent/contentLanguageCode should probably also
   accept a changed locale.

if(appliedConfigLocale !=== currentConfigLocale)
    updateAfterChangedContent

*/

    let initContent = (contentLanguageCode=null, locale=null) => {
        let rawColumnConfig = null
          , appliedConfigLocale = null
          ;

        if(contentLanguageCode !== null) {
            curentContentLanguageCode = contentLanguageCode;
            // overrides previous setting i.e. also resets
            // currentConfigLocale according to language setting
            currentConfigLocale = parseLocale(contentLanguageCode);
        }
        // Can be set together with contentLanguageCode, then it
        // overrides the result of parseLocale
        if(locale !== null)
            currentConfigLocale = locale;
        // else
        // we use the locale which is set by the widget
        // in fact currentConfigLocale is the source of truth of the widget

        // appliedConfigLocale is just information we want to
        // display, it's not stored as state, currentConfigLocale is the state
        [appliedConfigLocale, {
            columnConfig: rawColumnConfig
          , cssKeyframeFixes
          , justificationSpecs
          , wdthJustificationSpecs
        }] = selectTypeSpecByLocale(typeSpecI18N, currentConfigLocale);

        columnConfig = massageConfigurations?.columnConfig
            ? massageConfigurations.columnConfig(rawColumnConfig)
            : rawColumnConfig

        console.log('contentLanguageCode:', contentLanguageCode
                 , '\currentConfigLocale;', currentConfigLocale.map(e=>e && e.toString()).join('-')
                 , '\appliedConfigLocale;', appliedConfigLocale.map(e=>e && e.toString()).join('-')
                 );
    ...
    var updateViewportScheduled = null
      ...
      , updateAfterChangedContent = (contentLanguageCode, locale)=>{
            initContent(contentLanguageCode, locale);
    ...
    updateAfterChangedContent(curentContentLanguageCode);

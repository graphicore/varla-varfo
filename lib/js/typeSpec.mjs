...
export function localeToLabel(locale) {
    // Returns our local-logic style {script}-{language}-{territory}
    // but it excluded DFLT entries. an empty/all default locale is "DEFAULT"
    // e.g. "Latin-en", "Latin", "DEFAULT"
    return locale.filter(item=>(item!==DFLT && item !==null)).join('-') || 'default fallback'
}
export function localeToValue(locale) {
    // Returns our local-logic style {script}-{language}-{territory}
    // but it excluded DFLT entries. an empty/all default locale is "DEFAULT"
    // e.g. "Latin-en", "Latin", "DEFAULT"
    return locale.map(item=>(item===DFLT || item===null) ? '' : item).join('-');
}


function* deepKeys(obj, depth, path_=null) {
    const path = path_ === null
                ? []
                : path_
                ;

    for(const key of [...Object.getOwnPropertySymbols(obj), ...Object.keys(obj)]) {
        const childPath = [...path, key];
        if(childPath.length === depth)
            yield childPath;
        else
            yield* deepKeys(obj[key], depth, childPath);
    }
}
export function allLocalesFromTypeSpecI18N(typeSpecI18N) {
    // so far we only use columnConfigI18N
    return Array.from(deepKeys(typeSpecI18N.columnConfigI18N, 3));
}
...
// TODO: In these following cases where a reference of another config is
// used, we should define that explicitly like a link, so we could a)
// also return from the query which the original locale was, and we
// could prepare a storage format that is not javaScript (e.g. json)
// where object references are not directly possible.
//COLUMN_CONFIG[DFLT][DFLT][DFLT]
